...
func CreatePatchOperation(t *redsky.Trial, p *redsky.PatchTemplate, ref *corev1.ObjectReference, data []byte) (*redsky.PatchOperation, error) {
	...
	// If the patch is for the trial job itself, it cannot be applied
	// (since the job won't exist until well after patches are applied)
	if trial.IsTrialJobReference(t, ref) && p.Type != redsky.PatchStrategic {
